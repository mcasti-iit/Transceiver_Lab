#-------------------------------------------------------------
# Generated Example Tcl script for IP 'GTP_Zynq' (xilinx.com:ip:gtwizard:3.6)
#-------------------------------------------------------------

# Set up project params
set_param board.repoPaths D:/WinTools/Xilinx/Boards
# Declare source IP directory
set srcIpDir "d:/Projects/Transceiver/ZCB/GTestP_Zynq/GTestP_Zynq.srcs/sources_1/ip/GTP_Zynq"

# Create project
puts "INFO: \[open_example_project\] Creating new example project..."
create_project -name GTP_Zynq_ex -force
...
import_ip -files [list [file join $srcIpDir GTP_Zynq.xci]] -name GTP_Zynq
reset_target {open_example} [get_ips GTP_Zynq]
...
generate_target -quiet [_filter_supported_targets {instantiation_template synthesis simulation implementation shared_logic} [get_ips GTP_Zynq]] [get_ips GTP_Zynq]
...
add_files -scan_for_includes -quiet -fileset [current_fileset] \
  [list [file join $srcIpDir GTP_Zynq/example_design/gtp_zynq_exdes.vhd]] \
  [list [file join $srcIpDir GTP_Zynq/example_design/gtp_zynq_gt_frame_check.vhd]] \
  [list [file join $srcIpDir GTP_Zynq/example_design/gtp_zynq_gt_frame_gen.vhd]]
...
add_files -quiet -fileset [current_fileset] \
  [list [file join $srcIpDir GTP_Zynq/example_design/gt_rom_init_rx.dat]]
set_property USED_IN_SIMULATION false [get_files [list [file join $srcIpDir GTP_Zynq/example_design/gt_rom_init_rx.dat]]]
...
add_files -quiet -fileset [current_fileset -constrset] \
  [list [file join $srcIpDir GTP_Zynq/example_design/GTP_Zynq_exdes.xdc]]
...
add_files -quiet -scan_for_includes -fileset [current_fileset -simset] \
  [list [file join $srcIpDir GTP_Zynq/simulation/gtp_zynq_tb.vhd]] \
  [list [file join $srcIpDir GTP_Zynq/simulation/sim_reset_gt_model.vhd]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir GTP_Zynq/simulation/gtp_zynq_tb.vhd]]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir GTP_Zynq/simulation/sim_reset_gt_model.vhd]]]
...
add_files -quiet -fileset [current_fileset -simset] \
  [list [file join $srcIpDir GTP_Zynq/simulation/functional/gt_rom_init_rx.dat]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir GTP_Zynq/simulation/functional/gt_rom_init_rx.dat]]]
...
if {[catch {source [file join $srcIpDir GTP_Zynq/tcl/xci.tcl]} errMsg]} {
  puts "ERROR: \[open_example_project\] Open Example Project failed: Error encountered while sourcing custom IP example design script."
  puts "$errorInfo"
  error "ERROR: see log file for details."
  incr returnCode
}
if {[catch {source [file join $srcIpDir GTP_Zynq/tcl/gtp_zynq_partner.tcl]} errMsg]} {
  puts "ERROR: \[open_example_project\] Open Example Project failed: Error encountered while sourcing custom IP example design script."
  puts "$errorInfo"
  error "ERROR: see log file for details."
  incr returnCode
